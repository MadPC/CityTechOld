...
import com.madpcgaming.citytech.items.piping.AbstractPipingNetwork;
import com.madpcgaming.citytech.items.piping.ConnectionMode;
...
public class ItemPipingNetwork extends AbstractPipingNetwork<IItemPiping>
{
	...
	private final Map<BlockCoord, IItemPiping>			conMap			= new HashMap<BlockCoord, IItemPiping>();

	private boolean										requiresSort	= true;

	@Override
	public Class<? extends IItemPiping> getBasePipingType()
	{
		return IItemPiping.class;
	}

	@Override
	public void addPiping(IItemPiping con)
	{
		super.addPiping(con);
		...
		if (te != null) {
			for (ForgeDirection direction : con.getExternalConnections()) {
				...
				if (extCon != null) {
					inventoryAdded(con, direction, te.field_145851_c
							+ direction.offsetX, te.field_145848_d + direction.offsetY,
							te.field_145849_e + direction.offsetZ, extCon);
				}
			}
		}
	}

	public void inventoryAdded(IItemPiping itemPiping,
			ForgeDirection direction, int x, int y, int z,
			IInventory externalInventory)
	{
		BlockCoord bc = new BlockCoord(x, y, z);
		NetworkedInventory inv = new NetworkedInventory(externalInventory,
				itemPiping, direction, bc);
		inventories.add(inv);
		invMap.put(bc, inv);
		requiresSort = true;
	}
	...
	public ItemStack sendItems(ItemPiping itemPiping, ItemStack item,
			ForgeDirection side)
	{
	...
	{
		World world = itemPiping.getBundle().getEntity().worldObj;
	...
	{
		World world = piping.getBundle().getEntity().func_145831_w();
		if (world == null) {
			return;
		}
		if (world.isRemote) {
	...
	private void doTick(long tick)
	{
		...
		// System.out.println("Sortinging item network: took " + took + " nano "
		// + secs + " secs, " + (secs * 1000) + " millis");
		// }
	...
	class NetworkedInventory
	{
		...
		// work around for a vanilla chest changing into a double chest without
		// doing unnedded checks all the time
		boolean			recheckInv		= false;
		private int		delay			= 0;
		IInventory		origInv;

		NetworkedInventory(IInventory inv, IItemPiping con,
				ForgeDirection conDir, BlockCoord location)
		{
		...
		boolean canExtract()
		{
			...
			return mode == ConnectionMode.INPUT
		...
		{
			ConnectionMode mode = con.getConectionMode(conDir);
			return mode == ConnectionMode.OUTPUT
					|| mode == ConnectionMode.IN_OUT;
		}

		boolean isSticky()
		{
			return con.getOutputFilter(conDir).isValid()
		...
		{
			int transfered;
			if (tickDeficit > 0 || !canExtract()
					|| !con.isExtractionRedstoneConditionMet(conDir)) {
				// do nothing
			} else {
				transferItems();
			}
		...
		private boolean transferItems()
		{
			...
			if (recheckInv) {
				// Re-check vanilla chests twice a second to make sure they
				// haven't become double chests
				delay++;
				if (delay % 10 == 0) {
					inv = new InventoryWrapper(
							((InventoryWrapper) inv).getWrappedInv());
					delay = 0;
				}
			}
		...
		private boolean doTransfer(ItemStack extractedItem, int slot,
				int maxExtract)
		{
		...
		private int insertIntoTargets(ItemStack toExtract)
		{
			...
			for (Target target : sendPriority) {
				if (target.stickyInput && !matchedStickyInput) {
					ItemFilter of = target.inv.con
							.getOutputFilter(target.inv.conDir);
					matchedStickyInput = of.isValid()
							&& of.doesItemPassFilter(toExtract);
				}
				if (target.stickyInput || !matchedStickyInput) {
		...
		private int insertItem(ItemStack item)
		{
			...
			return ItemUtil.doInsertItem(inv, item,
					ForgeDirection.values()[inventorySide]);
		}
		...
		void updateInsertOrder()
		{
			...
			for (NetworkedInventory other : inventories) {
				if ((con.isSelfFeedEnabled(conDir) || (other != this))
						&& other.canInsert()
						&& con.getInputColor(conDir) == other.con
								.getOutputColor(other.conDir)) {

					if (Config.itemPipingUsePhysicalDistance) {
						sendPriority.add(new Target(other, distanceTo(other),
								other.isSticky()));
					} else {
						result.put(other.location, new Target(other, 9999999,
								other.isSticky()));
					}
				}
			}
		...
		private void calculateDistances(Map<BlockCoord, Target> result,
				Map<BlockCoord, Integer> visited, List<BlockCoord> steps,
				int distance)
		{
