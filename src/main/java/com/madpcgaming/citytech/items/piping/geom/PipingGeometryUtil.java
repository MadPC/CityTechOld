package com.madpcgaming.citytech.items.piping.geom;
...
import com.madpcgaming.citytech.lib.ForgeDirectionOffsets;
import com.madpcgaming.citytech.render.BoundingBox;
import com.madpcgaming.citytech.render.VecmathUtil;
import com.madpcgaming.citytech.render.Vector3d;

public class PipingGeometryUtil
{
	public static final PipingGeometryUtil instance = new PipingGeometryUtil();
	...
	public static void setupBounds(float scale)
	{
		...
		CORE_MAX = new Vector3d(CORE_MIN.x + WIDTH, CORE_MIN.y + HEIGHT,
				CORE_MIN.z + WIDTH);
		CORE_BOUNDS = new BoundingBox(CORE_MIN, CORE_MAX);

		float connectorWidth = 0.25f + (scale * 0.5f);
		for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
			EXTERNAL_CONNECTOR_BOUNDS.put(
					dir,
					createExternalConnector(dir, CONNECTOR_DEPTH,
							connectorWidth));
		}
	}

	private static BoundingBox[] createExternalConnector(ForgeDirection dir,
			float connectorDepth, float connectorWidth)
	{
	...
	private static BoundingBox createConnectorComponent(ForgeDirection dir,
			float cornerMin, float cornerMax, float depthMin, float depthMax)
	{
		float minX = (1 - Math.abs(dir.offsetX)) * cornerMin + dir.offsetX
				* depthMin;
		float minY = (1 - Math.abs(dir.offsetY)) * cornerMin + dir.offsetY
				* depthMin;
		float minZ = (1 - Math.abs(dir.offsetZ)) * cornerMin + dir.offsetZ
				* depthMin;

		float maxX = (1 - Math.abs(dir.offsetX)) * cornerMax
				+ (dir.offsetX * depthMax);
		float maxY = (1 - Math.abs(dir.offsetY)) * cornerMax
				+ (dir.offsetY * depthMax);
		float maxZ = (1 - Math.abs(dir.offsetZ)) * cornerMax
				+ (dir.offsetZ * depthMax);
	...
	private EnumMap<PipingConnectorType, BoundingBox>	connectorBounds	= new EnumMap<PipingConnectorType, BoundingBox>(
																				PipingConnectorType.class);

	private PipingGeometryUtil()
	{
	}
	...
	public BoundingBox getBoundingBox(Class<? extends IPiping> type,
			ForgeDirection dir, boolean isStub, Offset offset)
	{
	...
	{
		Vector3d result = new Vector3d(offset.xOffset, offset.yOffset,
				offset.zOffset);
		result.scale(WIDTH);
		return result;
	}

	public BoundingBox createBoundsForConnectionController(ForgeDirection dir,
			Offset offset)
	{
	...
	private BoundingBox createConduitBounds(Class<? extends IPiping> type,
			GeometryKey key)
	{
			...
			ForgeDirection dir, boolean isStub, Offset offset)
	{
		...
		min.add(trans);
		max.add(trans);
		bb = new BoundingBox(VecmathUtil.clamp(min, 0, 1), VecmathUtil.clamp(
				max, 0, 1));
		return bb;
	}
