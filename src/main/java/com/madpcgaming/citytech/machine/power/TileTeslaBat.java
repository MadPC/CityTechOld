		...
		IInventory
{
	...
	private Object getReceptorForFace(ForgeDirection faceHit)
	{
		...
		TileEntity te = worldObj.getTileEntity(checkLoc.x, checkLoc.y,
				checkLoc.z);
	...
	public void updateEntity()
	{
		...
		requiresClientSync |= lastSyncPowerStored != storedEnergy
	...
	{
		if (gaugeBounds == null) {
			gaugeBounds = GaugeBounds.calculateGaugeBounds(
					new BlockCoord(this), multiblock);
		}
		return gaugeBounds;
	}
		...
		for (ItemStack item : inventory) {
			if (item != null && available > 0) {
				...
				if (item.getItem() instanceof IEnergyContainerItem) {
					IEnergyContainerItem chargable = (IEnergyContainerItem) item
							.getItem();
					...
					if (cur < max) {
						used = chargable.receiveEnergy(item, (int) canUse,
								false) / 10;
					...
					IChargeableItem chargable = (IChargeableItem) item
							.getItem();
					float max = chargable.getMaxEnergyStored(item);
					float cur = chargable.getEnergyStored(item);
					...
					if (cur < max) {
						if (chargable.getClass().getName().startsWith("appeng")
								|| "appeng.common.base.AppEngMultiChargeable"
										.equals(chargable.getClass().getName())) {
							// 256 max limit
	...
	public boolean isOutputEnabled(ForgeDirection direction)
	{
		...
		return mode == FaceConnectionMode.OUTPUT
				|| mode == FaceConnectionMode.NONE && isOutputEnabled();
	}
	...
	{
		FaceConnectionMode mode = getFaceModeForFace(direction);
		return mode == FaceConnectionMode.INPUT
				|| mode == FaceConnectionMode.NONE && isInputEnabled();
	}
	...
	private boolean transmitEnergy()
	{
		...
		while (receptorIterator.hasNext() && canTransmit > 0
				&& appliedCount < numReceptors) {

			Receptor receptor = receptorIterator.next();
			IPowerInterface powerInterface = receptor.receptor;
			FaceConnectionMode mode = receptor.mode;
			if (powerInterface != null
					&& mode != FaceConnectionMode.INPUT
					&& mode != FaceConnectionMode.LOCKED
					&& powerInterface.getMinEnergyReceived(receptor.fromDir
							.getOpposite()) <= canTransmit) {
				float used;
				if (receptor.receptor.getDelegate() instanceof IPipingBundle) {
					IPipingBundle bundle = (IPipingBundle) receptor.receptor
							.getDelegate();
					IPowerPiping piping = bundle.getPiping(IPowerPiping.class);
					if (piping != null
							&& piping.getConnectionMode(receptor.fromDir
									.getOpposite()) == ConnectionMode.INPUT) {
						used = powerInterface.receiveEnergy(
								receptor.fromDir.getOpposite(), canTransmit);
					}
					else {
						used = 0;
					}
			...
			if (mode != FaceConnectionMode.LOCKED) {
				...
				if (!(te instanceof TileTeslaBat)) {
					...
					if (ph != null && ph.canPipingConnect(dir)) {
						...
						if (mode == FaceConnectionMode.NONE
								&& !(ph.getDelegate() instanceof IInternalPowerReceptor)) {
	...
	public PowerReceiver getPowerReceiver(ForgeDirection side)
	{
		...
		if (mode == FaceConnectionMode.LOCKED
				|| mode == FaceConnectionMode.OUTPUT) {
	...
	public int doReceiveEnergy(ForgeDirection from, int maxReceive,
			boolean simulate)
	{
	...
	private boolean isRecievingRedstoneSignal()
	{
		...
		if (!isMultiblock()) {
			isRecievingRedstoneSignal = worldObj.getStrongestIndirectPower(
	...
	PowerHandler doGetPowerHandler()
	{
		if (inputEnabled) {
			if (powerHandler == null) {
				powerHandler = PowerHandlerUtil.createHandler(
						new BasicTeslaBat(maxInput, maxInput, maxOutput), this,
						Type.STORAGE);
			}
			return powerHandler;
		}
	...
	private PowerHandler getDisabledPowerHandler()
	{
		if (disabledPowerHandler == null) {
			disabledPowerHandler = PowerHandlerUtil.createHandler(
					new BasicTeslaBat(0, 0), this, Type.STORAGE);
		}
	...
	{
		return VecmathUtil.clamp(
				Math.round(scale * (storedEnergy / maxStoredEnergy)), 0, scale);
	}
	...
	private void findNighbouringBanks(TileTeslaBat tileTeslaBat,
			List<TileTeslaBat> blocks)
	{
		...
		if (isMaster()) {
			...
			if (invItems.size() > inventory.length) {
				for (int i = inventory.length; i < invItems.size(); i++) {
					Util.dropItems(worldObj, invItems.get(i), xCoord, yCoord,
							zCoord, true);
				}
			}
	...
	public void readFromNBT(NBTTagCompound nbtRoot)
	{
		...
		if (maxStoredEnergy != 0
				&& Math.abs(oldEnergy - newEnergy) / maxStoredEnergy > 0.05
		...
		inputControlMode = RedstoneControlMode.values()[nbtRoot
				.getShort("inputControlMode")];
		outputControlMode = RedstoneControlMode.values()[nbtRoot
				.getShort("outputControlMode")];
		...
		if (nbtRoot.getBoolean("isMultiblock")) {
			...
			for (int i = 0; i < multiblock.length; i++) {
				multiblock[i] = new BlockCoord(coords[c++], coords[c++],
						coords[c++]);
			}
		...
		NBTTagList itemList = nbtRoot.getTagList("Items", 0);
		for (int i = 0; i < itemList.tagCount(); i++) {
			NBTTagCompound itemStack = (NBTTagCompound) itemList.getCompoundTagAt(i);
		...
		if (nbtRoot.hasKey("hasFaces")) {
			for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
				if (nbtRoot.hasKey("face" + dir.ordinal())) {
					setFaceMode(
							dir,
							FaceConnectionMode.values()[nbtRoot.getShort("face"
									+ dir.ordinal())], false);
				}
			}
		}
	...
	public void writeToNBT(NBTTagCompound nbtRoot)
	{
		...
		nbtRoot.setShort("outputControlMode",
				(short) outputControlMode.ordinal());
		...
		if (faceModes != null) {
			...
			for (Entry<ForgeDirection, FaceConnectionMode> e : faceModes
					.entrySet()) {
				nbtRoot.setShort("face" + e.getKey().ordinal(), (short) e
						.getValue().ordinal());
			}
		}
	...
	static class Receptor
	{
		...
		{
			return "Receptor [receptor=" + receptor + ", fromDir=" + fromDir
					+ ", mode=" + mode + "]";
		}
