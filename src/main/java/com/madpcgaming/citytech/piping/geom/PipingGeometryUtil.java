...
public class PipingGeometryUtil
{
	...
	private EnumMap<PipingConnectorType, BoundingBox> connectorBounds = new EnumMap<PipingConnectorType, BoundingBox>(PipingConnectorType.class);
	...
	public BoundingBox getBoundingBox(Class<? extends IPiping> type, ForgeDirection dir, boolean isStub, Offset offset)
	{
		GeometryKey key = new GeometryKey(dir, isStub, offset, type);
	    BoundingBox result = boundsCache.get(key);
	    if(result == null) 
	    {
	      result = createPipingBounds(type, key);
	...
	private BoundingBox createPipingBounds(Class<? extends IPiping> type, GeometryKey key)
	{
	    return createPipingBounds(type, key.dir, key.isStub, key.offset);
	  }

	  private BoundingBox createPipingBounds(Class<? extends IPiping> type, ForgeDirection dir, boolean isStub, Offset offset) {
	    BoundingBox bb = CORE_BOUNDS;
